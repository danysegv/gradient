// Shared by the extension's own pages: popup, options and panel.

export const ext = globalThis.browser ?? globalThis.chrome;

/** Asks the background; always resolves. */
export async function ask(type, payload = {}) {
  try {
    const res = await ext.runtime.sendMessage({ type, ...payload });
    return res ?? { ok: false, error: "No answer from the extension." };
  } catch (err) {
...
/** The locked wordmark (components/wordmark.tsx), built node by node. */
...
export function mountMarks() {
  for (const el of document.querySelectorAll("[data-mark]")) el.replaceChildren(wordmark());
}

/** The sign-in form, shared by the popup and the options page. */
export function wireSignIn(form, { onSignedIn }) {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    if (res.ok) onSignedIn(res.name);
    else error.textContent = res.error || "Something went wrong. Try again.";
  });
}
