...
- **Clipper tool**: the "paste a URL" form on `/clip`, plus the **04AM Clipper browser extension** (`extension/`, since 2026-09-23; Chrome, Edge, Firefox, Safari from one MV3 codebase — see `extension/README.md`)
...
- **2026-09-11 — Curator names are public; the clip detail page is public again.** Daniela decided curator identity is public. The noindex lines on `/curator/[name]`, `/curators`, `/trend/[name]` and `/clip/[id]` were removed together. This **supersedes the 2026-08-25 rule** that confined `clipped_by_name` to `/clip`: public pages may select and render curator names. It does *not* relax the person layer — `curator_identities` (which name belongs to which person) stays unreadable by `anon`, and only aggregates leave `panel_composition`. Separately: Next 16 calls middleware **`proxy.ts`**, and it existed all along. Its matcher was `["/clip", "/clip/:path*"]`, which put the public `/clip/<uuid>` page behind the password form from 2026-09-04 to 2026-09-11 — the 09-04 note "there is no middleware" was wrong because it looked for `middleware.ts`. The matcher is now exactly `["/clip"]`, and `/clip` re-checks the session in its own page component. Never widen that matcher.
- **2026-09-11 — Search replaces the filter chips; Claude describes every image, for search only.** One search bar on `/`, `/curator/[name]` and board pages; the query lives in the URL (`?q=`) and the server matches it. Clips are ranked by `search_clips` over tag names (CamelCase split) and universal terms at confidence >= 0.5, **Claude's description of the image and its keywords**, the title, and caption/credits. Every word must match, with the last word as a prefix; if nothing matches every word, clips matching any word come back flagged so the page says so. Boards match on title and description via `search_boards` (RLS decides: public boards, plus a signed-in curator's own). Descriptions live in **`clip_descriptions`, never `clip_tags`**: no axis, no confidence, never in any count, share, velocity or panel drift, so describing the whole library moves no figure and is safe before the 09-26 launch. **Decided search-only:** the table has RLS on, no policy and no grants to anon/authenticated; `search_clips` is SECURITY DEFINER and returns clip ids and a rank, never text (same boundary pattern as `panel_composition`). `lib/search/search.test.ts` fails if anything but `lib/claude/describe-clip.ts` names the table. New clips are described after classification in `app/clip/actions.ts` (a failure there never affects tagging); the backlog is described in batches of 20 from the button on `/clip` (probe-first, unreadable images park, exactly like reclassify). The old faceted chips (`lib/facets.ts`) and the trend page's co-tag rail are removed.

- **2026-09-23 — Browser extension: a second door, same room.** `extension/` is one MV3 codebase for Chrome/Edge/Firefox/Safari (no build step to load unpacked; `node extension/build.mjs` makes per-store zips). Curators only: it signs in with the 04AM account (email + password) through `app/api/extension/session`, which hands back the account's Supabase tokens **only if** `my_curator_profile` returns a row — any other account is signed straight back out. The extension sends `Authorization: Bearer <access token>`; `lib/clip-session.ts` gained `getBearerSession()` and stays the ONE session door (`lib/auth/auth.test.ts` fails if anything else reads the header or calls `my_curator_profile`). Legacy password curators can't use it until their account is linked. **One insert for both doors:** the body of `createClip` moved unchanged to `lib/clips/create.ts` (`insertClip`, enrichment in `after()`), and validation to `lib/clips/clip-input.ts` (`parseClipInput`, pure, tested) — `app/clip/actions.ts` and `app/api/extension/clip/route.ts` both call them, so they accept exactly the same clips. The taxonomy-freeze guard was updated to match: `app/clip/actions.ts` no longer names `classifyAndTagClip`; a new test pins it to `lib/clips/create.ts`, only on the row that call just inserted, reachable from exactly those two files. **Link-only, enforced:** `parseClipInput` now refuses non-http(s) page/image URLs, including `data:`/`blob:` (would mean storing a copy — §512(c)) and `javascript:`; the extension never uploads, screenshots or crops, and prefers the largest `srcset` candidate the page offers. **Credits the page publishes are suggestions, never pre-filled**, so `attribution_parsed_at` keeps meaning "inferred"; `found_via` comes from the server's `foundViaFromUrl`, and a blank one is filled by the classifier as before. **Duplicates are blocked** in the panel (same `image_url` already active) because a second copy would count twice in every figure — that is arithmetic, not moderation; there is still no approve-before-publish queue. Permissions: `activeTab` + `scripting` (page script injected only on a clip action), `contextMenus`, `storage`, one host permission for the production origin; CORS on `/api/extension/*` answers extension origins only, and nothing there is cookie-authenticated. `proxy.ts` untouched.
